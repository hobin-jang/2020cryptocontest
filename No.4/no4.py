...
cipher=[[0] for i in range(1025)] #cipher : encrypted파일 16byte 씩 블록 만든 배열
...
for i in range(1025):
        ...
        power[i]=power[i].values

power=np.array(power)

#key_10,key_9,key_8 : 10,9,8라운드 라운드 키 (미리 구할 수 있는 애들 적어놓은 거)
#key_10=[[0x22,0xE5,None,0xA1],[0xE9,0x39,None,0x4C],[None,0xA5,0x5B,0x2C],[None,0x9B,0x3E,None]]
#key_9=[[0xA6,0xA8,0x6B,0x63],[0xCB,0xDC,0x8B,0xED],[None,0x9C,None,0x60],[0xA8,0x3E,0x65,None]]
#key_8=[[0x87,None,None,None],[0x6D,0x74,0xE0,0x8E],[None,0x40,None,0x8D],[None,0xA2,None,None]]

key_10=[[0x22,0xE5,None,None],[0xE9,None,None,0x4C],[None,None,0x5B,0x2C],[None,None,0x9B,None]]

#9라운드 키 첫 번 째 키 찾는 과정
cipher2=[[0]for i in range(1025)]

for i in range(1025):
    x=cipher[i][0]^key_10[0][0]  #shiftrow 시켜서 9라운드 키 첫번째 열이랑 맞는 위치들로 계싼
    y=cipher[i][13]^key_10[3][1]
    z=cipher[i][10]^key_10[2][2]
    w=cipher[i][7]^key_10[1][3]
    x=isbox[x]
    y=isbox[y]
    z=isbox[z]
    w=isbox[w]
