#include <iostream>
#include "decryptor.h"

int main()
{
    int i, j;
    int Hex;
    Byte IV[16] = { 0x22, 0x75, 0x7D, 0x72, 0x10, 0xD2, 0x1B, 0x07, 0x5A, 0xAE, 0xDE, 0x01, 0x64, 0x5E, 0x84, 0x27 };
    Byte c1[] = {
       0xC9, 0xA7, 0x35, 0xF6, 0xAC, 0x21, 0xEA, 0x31, 0x80, 0xE9, 0x1C, 0x33, 0x0F, 0x82, 0x5D, 0x46,
       /**/
    }; //각 줄이 16바이트 씩으로 나누어져 있어 필요한 16바이트 블록에 대한 부분을 제외한 나머지 줄은 주석처리하여 디버깅

    for (j = 15; j >= 0; j--)
    {
        for (i = 0; i < 256; i++)
        {
            IV[j] = i; //IV의 값을 0~255까지 조사하는 부분
            int ret;

            struct CTX ctx1;

            ctx1.ciphertext = c1;
            ctx1.cipher_length = sizeof(c1);
            ctx1.IV = IV;

            ret = Dec_CTX(&ctx1); //0~255까지 조사되는 IV값에 대해 ret 값이 0이면 패딩이 올바르게 된 것

            if (ret == 0)
            {
                printf("%d\n", j);
                IV[j] = i ^ 128;
                printf("G : %d\n", i);
                printf("VALID\n\n");
                break;
            } // 16 바이트 블록에 대해 전수조사하여 valid한 값 16바이트를 찾음
        }
    }
    return 0;
}